...
public class Card : MonoBehaviour
{
    ...
    void Start()
    {
        myManager = W6GameManager.FindInstance(); //get the game manager
    ...
    void OnMouseDown() { //when the mouse is clicked
        ...
        if (myManager.currentState == W6GameManager.State.Select) //if we're in the select state
        {
            if (currentState == State.FaceDown) //if the card is face down
            {
                TransitionState(State.FaceUp); //move to the faceup state
                myManager.TransitionStates(W6GameManager.State.CardChoosen); //move to cardchoosen state
